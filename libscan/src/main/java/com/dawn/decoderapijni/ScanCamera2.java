...
public class ScanCamera2 implements ICamera {
    ...
    public ScanCamera2(Context context, int previewFormat) {
        Log.d(TAG,"New instance ....");
        mCameraManager = (CameraManager)context.getSystemService(Context.CAMERA_SERVICE);
        mPreviewFormat = previewFormat;
        // 创建后台工作线程，初始化可以用于向后台线程发送消息的 Handler
        mBackgroundThread = new HandlerThread("ScanCamera2");
    ...
    ImageReader.OnImageAvailableListener onImage = new ImageReader.OnImageAvailableListener() {
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            yBuffer.get(buf);
            image.close();
            Log.i(TAG,"++++++++++++++++ onPreviewFrame ++++++++++  bufsize : "+ buf.length);
            SoftEngine.getInstance()
                    .setSoftEngineIOCtrlEx(SoftEngine.JNI_IOCTRL_SET_DECODE_IMG, buf.length, buf);
        ...
        try {
            ...
            synchronized (mCameraDevice) {
                ...
                if (mCameraDevice.device != null) {
                    builder = mCameraDevice.device.createCaptureRequest(CameraDevice.TEMPLATE_PREVIEW);
                }
            }
            if (builder!=null) {
...
//            try {
//                mCameraCaptureSession.session.stopRepeating();
